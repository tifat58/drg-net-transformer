...
from models.GAIN_Attention import GAIN
...
def train(epoch, model, criterion, optimizer, train_loader, max_epoch,  tbWriter):
    ...
    for batch_idx, (inputs, labels_multiclass, labels_onehot, features) in enumerate(train_loader):
        ...
        logits, logits_am, heatmap = model(inputs, targets_class, features_true)
    ...
    print(
        '\t Train:    Acc %0.3f,  AUC %0.3f, weightedKappa %0.3f, weightedF1 %0.3f, loss1 %.6f, loss2 %.6f, time %3.2f'
...
def val(epoch, model, criterion, val_loader, max_epoch, tbWriter):
    ...
    end_time = time.time()
    print('-------- Epoch {}/{}'.format(epoch, max_epoch))
    print('-------- Val:  Acc %0.3f,  AUC %0.3f, weightedKappa %0.3f, weightedF1 %0.3f, loss %.6f, loss1 %.6f, loss2 %.6f, time %3.2f'
        % (accuracy, AUC, wKappa, wF1, losses.avg, losses1.avg, losses2.avg, end_time - start_time))
    print('=============================================================')
    return AUC, wF1
