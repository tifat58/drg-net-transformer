...
def main():
    ...
    # save_model_path = r'/mnt/sda/haal02-data/results/MIL-VT_results/fgadr_new'
    save_model_path = r'/mnt/sda/haal02-data/results/MIL-VT_results/fgadr_new_transformer'
    ...
    #
    dataset_train = dataset_fgadr(data_path, DF_train, full_seg_path=full_seg_path, transform=transform_train_simple,
                                  transform_mask=transform_mask)
    dataset_valid = dataset_fgadr(data_path, DF_val, full_seg_path=full_seg_path, transform=transform_test,
                                  transform_mask=transform_mask)
    dataset_test = dataset_fgadr(data_path, DF_test, full_seg_path=full_seg_path, transform=transform_test,
                                 transform_mask=transform_mask)
...
def train(epoch, model, criterion, optimizer, train_loader, max_epoch,  tbWriter):
    ...
    for batch_idx, (inputs, labels_multiclass, labels_onehot, features) in enumerate(train_loader):
        ...
        outputs_class, outputs_MIL, Ac = model(inputs, targets_class)
        loss1 = criterion(outputs_class, targets_class)
        loss2 = criterion(outputs_MIL, targets_class)

        loss3 = jaccard_loss(features_true, Ac)
        # print(loss3)
    ...
    print(
        '\t Train:    Acc %0.3f,  AUC %0.3f, weightedKappa %0.3f, weightedF1 %0.3f, loss1 %.6f, loss2 %.6f, loss3 %.6f, time %3.2f'
...
def val(epoch, model, criterion, val_loader, max_epoch, tbWriter):
    ...
    for batch_idx, (inputs,  labels_multiclass, labels_onehot, features) in enumerate(val_loader):
        ...
        outputs_class = model(inputs, targets_class)
