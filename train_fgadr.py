...
def main():
    ...
    #
    dataset_train = dataset_fgadr(data_path, DF_train, transform = transform_train)
    dataset_valid = dataset_fgadr(data_path, DF_val, transform = transform_test)
    dataset_test = dataset_fgadr(data_path, DF_test, transform=transform_test)
...
def train(epoch, model, criterion, optimizer, train_loader, max_epoch,  tbWriter):
    ...
    print('\t Train:   AUC %0.3f, weightedKappa %0.3f, weightedF1 %0.3f, loss1 %.6f, loss2 %.6f, time %3.2f'
...
def val(epoch, model, criterion, val_loader, max_epoch, tbWriter):
    ...
    end_time = time.time()
    print('-------- Epoch {}/{}'.format(epoch, max_epoch))
    print('-------- Val:  AUC %0.3f, weightedKappa %0.3f, weightedF1 %0.3f, loss %.6f, time %3.2f'
        % (AUC, wKappa, wF1, losses.avg,  end_time - start_time))
    print('=============================================================')
    return AUC, wF1
