...
import utils
from data_manager.dataset import dataset_Aptos, dataset_eyepacs, dataset_idrid
from loss.MultiClassMetrics import *
from models.FinetuneVTmodels_MOD import MIL_VT_FineTune
from utils import *
torch.multiprocessing.set_sharing_strategy('file_system')

from util_folder.preprocess import visualize_predict
####################################
...
def generate_explantion(model, activations, prediction, threshold=0.5):
...
def main():
    ...
    """Basic Setting"""
    data_path = r'/mnt/sda/haal02-data/IDRID-updated/DiseaseGrading/OriginalImages/TrainingSet'
    csv_path = r'/mnt/sda/haal02-data/IDRID-updated/DiseaseGrading/Groundtruths/'

    test_data_path = r'/mnt/sda/haal02-data/IDRID-updated/DiseaseGrading/OriginalImages/TestingSet'

    save_model_path = r'/mnt/sda/haal02-data/results/MIL-VT_results/IDRID_new'
    csvName = csv_path + 'IDRiD_Disease_Grading_Training_Labels_renamed.csv'  ##the csv file store the path of image and corresponding label
    test_csvName = csv_path + 'IDRiD_Disease_Grading_Testing_Labels_renamed.csv'
    ...
    DF_val = DF1.loc[test_indexes[:valNum]]
    DF_test = DF1.loc[test_indexes[:valNum]]
    ...
    #
    dataset_train = dataset_idrid(data_path, DF_train, transform = transform_train)
    dataset_valid = dataset_idrid(test_data_path, DF_val, transform = transform_test)
    dataset_test = dataset_idrid(test_data_path, DF_test, transform=transform_test)
...
def train(epoch, model, criterion, optimizer, train_loader, max_epoch,  tbWriter):
    ...
    for batch_idx, (inputs, labels_multiclass, labels_onehot) in enumerate(train_loader):
        ...
        activation_model = copy.deepcopy(model)
        # h1 = activation_model.blocks[0].attn.proj_drop.register_forward_hook(getActivation('attn_drop'))
        # issue here
        outputs_class, outputs_MIL = model(inputs)

        attention = activation_model.get_last_selfattention(inputs)
        attention_output_class, outputs_MIL = activation_model(inputs)
        ...
        a, b = generate_explantion(activation_model, attention, attention_output_class, threshold=0.5)
        # print(a.shape)
        exit()
